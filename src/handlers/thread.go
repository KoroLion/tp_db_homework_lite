...
func ThreadCreate(c echo.Context) error {
    ...
    var forumId int
    err = db.QueryRow(
        "UPDATE forums SET threads = threads + 1 WHERE LOWER(slug) = LOWER($1) RETURNING id, slug",
        newThread.Forum,
    ...
    err = db.QueryRow(
        "INSERT INTO threads (forum, title, author, message, created, slug) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id",
